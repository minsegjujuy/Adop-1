...
class DocumentoViewSet(viewsets.ModelViewSet):
        ...
        if Vigilancia.objects.filter(id=kwargs["vigilancia_id"]):
            ...
            if documentos:
                ...
                for documento in documentos:
                    ...
                    resp.append(doc)
                return JsonResponse(
                    {"archivos": resp}, safe=False, status=status.HTTP_200_OK
                )
            return JsonResponse(
                {"msj": "No hay Documentos Cargados."},
                safe=False,
                status=status.HTTP_200_OK,
            )
        return JsonResponse(
            {"error": "La Vigilancia No Existe."}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            try:
                ...
                return JsonResponse(
                    {
                        "error": f"No existe la vigilancia de ID: {kwargs['vigilancia_id']}."
                    }
                )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if errors:
            return JsonResponse(
                {"error": f"Los archivos {errors}ya estan cargados en el sistema."},
                status=status.HTTP_409_CONFLICT,
            )

        headers = self.get_success_headers()
        return JsonResponse(
            {"msj": "Archivo/s guardado/s correctamente!!"},
    ...
    def update(self, request, *args, **kwargs):
        try:
            direccion = (
                MEDIA_ROOT + f"/vigilancias/vigilancia-{kwargs['vigilancia_id']}"
            )
            if path.exists(direccion):
                ...
                if path.exists(documento.file.path):
                    if not path.exists(f"{direccion}/{request.FILES['file'].name}"):
                        ...
                        return JsonResponse(
                            {"error": "Este archivo ya esta cargado en el sistema"},
                ...
                return JsonResponse(
                    {
                        "error": f"El archivo no existe en la vigilancia {kwargs['vigilancia_id']}"
                    },
